...
export default function ProjectBillPDF({ data, currency = 'USD' }) {
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.section}>
          ...
          <Text style={{ color: '#888', fontSize: 10 }}>{project.wage_type === 'prevailing' ? 'Prevailing Wages' : 'Regular Wages'}</Text>
        </View>
        ...
        <View style={s.section}>
          ...
          {summary.regular_hours > 0 && (
            <View style={s.costRow}>
              <Text style={{ color: '#2563eb' }}>Regular ({summary.regular_hours.toFixed(2)}h)</Text>
              <Text style={{ color: '#2563eb', fontWeight: 'bold' }}>{formatCurrency(summary.regular_cost, currency)}</Text>
            </View>
          )}
          {summary.overtime_hours > 0 && (
            <View style={s.costRow}>
              <Text style={{ color: '#dc2626' }}>Overtime ({summary.overtime_hours.toFixed(2)}h × 1.5x)</Text>
        ...
        <View style={s.section}>
          ...
          <View style={s.headerRow}>
            <Text style={[s.colWide, s.bold]}>Worker</Text>
